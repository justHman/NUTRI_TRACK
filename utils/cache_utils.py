...
from config.client_config import _CACHE_TTL_DAYS
...
def is_expired(entry: dict) -> bool:
    ts = entry.get("_ts", 0)
    return (get_now_ts() - ts) > (_CACHE_TTL_DAYS * 86400)
...
def save_disk_cache(cache: dict, cache_file: str, cache_dir: str, cache_key=None):
    ...
    try:
        ...
        logger.debug("L2 cache saved: %d entries (%d foods, %d barcodes) to %s",
                     ...
                     len(cache.get("foods", {})), len(cache.get("barcodes", {})), cache_file)
...
if __name__ == "__main__":
    data, raw = load_disk_cache(r"D:\Project\Code\nutritrack-documentation\app\data\avocavo_cache.json")
    import json
    with open(r"D:\Project\Code\nutritrack-documentation\app\data\raw.json", "w", encoding="utf-8") as f:
        json.dump(raw, f, ensure_ascii=False, indent=2)
    with open(r"D:\Project\Code\nutritrack-documentation\app\data\data.json", "w", encoding="utf-8") as f:
