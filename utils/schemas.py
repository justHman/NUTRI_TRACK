...
class NutritionItem(BaseModel):
    ...
    value: float
    unit: str
    dv_percentage: Optional[float] = Field(
        default=None, description="Daily value percentage, if available"
    )
...
class LabelItem(BaseModel):
    ...
    expiry_days: Optional[int] = Field(
        default=None,
        description="Number of days the food can be used before expiration, measured in days",
    )
...
class LabelList(BaseModel):
    ...
    image_quality: Optional[str] = Field(
        default=None,
        description="Image quality: good | poor_lighting | blurry | partial_view",
    )
...
class Ingredient(BaseModel):
    ...
    weight: Optional[float] = Field(
        default=None, description="Estimated weight in grams"
    )
    nutritions: Optional[NutritionInfo] = Field(
        default=None, description="Estimated nutrition"
    )
    confidence: Optional[float] = Field(
...
class FoodItem(BaseModel):
    ...
    confidence: Optional[float] = Field(
        default=None, description="Confidence score for dish identification (0.0-1.0)"
    )
    cooking_method: Optional[str] = Field(
        default=None,
        description="Cooking method: grilled | fried | steamed | boiled | raw | mixed",
    )
    ...
    nutritions: Optional[NutritionInfo] = Field(
        default=None, description="Total estimated nutrition"
    )
    expiry_days: Optional[int] = Field(
        description="Number of days the food can be used before expiration, measured in days"
    )
    scale_reference: Optional[str] = Field(
        default=None,
        description="What was used as scale reference: chopsticks visible | plate size | no reference",
    )
