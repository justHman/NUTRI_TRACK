        ...
        if r is None:
            return [
                (
                    ...
                    "returned None (even with DEMO_KEY)",
                )
            ]
...
def _test_cache_l2_hit(client) -> list:
    try:
        from third_apis.AvocavoNutrition import (
            MISSING,
            AvocavoNutritionClient,
            _l1_foods,
            _l2,
            get_now_ts,
        )

        query = "__l2test_chicken_avocavo__"
        fake_food = {
            ...
            "parsing": {
                ...
                "ingredient_name": "Test Chicken L2 Avocavo",
            },
        }
        _l2["foods"][query] = {
            "food": fake_food,
        ...
        assert (
            ...
            and l1_val.get("ingredient") == "Test Chicken L2 Avocavo"
        )
        ...
        _l2["foods"].pop("__l2test_chicken_avocavo__", None)
        AvocavoNutritionClient.clear_l1_cache()
...
def _test_expired_cache_entries(client) -> list:
    ...
    try:
        ...
        _l2["foods"].pop("__expiredtest_avocavo__", None)
        AvocavoNutritionClient.clear_l1_cache()
...
def _test_mock_data() -> list:
    ...
    for key_val in ("DEMO_KEY", "", None):
        mock_client = AvocavoNutritionClient(api_key=key_val)
...
def _test_malformed_response_handling(client) -> list:
    ...
    test_client = AvocavoNutritionClient(
        api_key="DEMO_KEY"
    )  # Use demo for predictable behavior
...
def _test_cache_lru_eviction(client) -> list:
    ...
    try:
        from third_apis.AvocavoNutrition import AvocavoNutritionClient, _l1_foods
        ...
        from third_apis.AvocavoNutrition import MISSING
...
def run_all(client) -> list:
    ...
    try:
        print(
            "\n─── Avocavo Nutrition Client Tests ────────────────────────────────────",
            flush=True,
        )
...
def test_avocavonutrition_client_suite():
    from third_apis.AvocavoNutrition import AvocavoNutritionClient

    client = AvocavoNutritionClient(
        api_key=os.getenv("AVOCAVO_NUTRITION_API_KEY", "FAKE_TEST_KEY")
    )
    group_results = run_all(client)
    assert all(group_results), f"Avocavo client suite failed: {group_results}"
