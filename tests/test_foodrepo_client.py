"""
Tests for FoodRepo Client
==========================
Tests the FoodRepo API client: search, nutrition, ingredients, caching.
"""
...
def _test_cache_l2_hit(client) -> list:
    try:
        from third_apis.FoodRepo import _l2, _l1, _now_ts, _MISSING, FoodRepoClient
        query = "__l2_test_chicken_fr__"
        fake_food = {
            "id": 999999,
            "display_name_translations": {"en": "Test Chicken L2 FR"},
            "nutrients": {
                "energy":        {"per_hundred": 165.0, "unit": "kcal"},
                "protein":       {"per_hundred": 31.0,  "unit": "g"},
                "fat":           {"per_hundred": 3.6,   "unit": "g"},
                "carbohydrates": {"per_hundred": 0.0,   "unit": "g"},
            },
            "ingredients_translations": {"en": "CHICKEN BREAST, SALT, WATER"},
        }
        _l2[query] = {"food": fake_food, "_ts": _now_ts()}
        FoodRepoClient.clear_l1_cache()
        assert _l1.get(query) is _MISSING
        start = time.time()
        r = client.search_best(query)
        elapsed = time.time() - start
        assert r is not None
        name = r.get("display_name_translations", {}).get("en", "")
        assert name == "Test Chicken L2 FR"
        l1_val = _l1.get(query)
        assert l1_val is not _MISSING
        return [(True, "synthetic inject+promote", f"{elapsed:.4f}s  L1 promoted ✓")]
    except Exception as e:
        return [(False, "synthetic inject+promote", str(e))]
    finally:
        from third_apis.FoodRepo import _l2, FoodRepoClient
        _l2.pop("__l2_test_chicken_fr__", None)
        FoodRepoClient.clear_l1_cache()
...
def _test_mock_data(client) -> list:
    ...
    mock_client = FoodRepoClient(api_key="DEMO_KEY")
...
def _test_search_by_barcode(client) -> list:
    """Test search_by_barcode() returns raw FoodRepo JSON response shape."""
    cases = [
        ("abc", False),
    ]
    results = []
...
def run_all(client) -> list:
    """Run all FoodRepo client tests.

    Args:
        client: Pre-initialized FoodRepoClient instance
    ...
    try:
        print("\n─── FoodRepo Client Tests ─────────────────────────────────────────────", flush=True)
