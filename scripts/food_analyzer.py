"""
NutriTrack Food Analyzer Script
==========================
Pipeline: Image → ANALYSIST (analyze_food) → USDA Nutrition Lookup → Final Results

Supports 2 modes:
- "tools": Model drives USDA lookups via tool calling (Method 3)
- "manual": Traditional 2-step flow — ANALYSIST → USDA per ingredient (Method 1 + USDA)

Usage:
    python -m app.scripts.food_analyzer <image_path> [--method tools|manual]
"""
...
from utils.caculator import (
    adjust_nutrition_for_cooking_method,
...
def analyze_food_nutrition(
    ...
    analysist: Optional[ANALYSIST] = None,
    ...
    """
    Full pipeline: Image → ANALYSIST → USDA → Nutrition Results
    """
    ...
    if analysist is None:
        logger.info("Initializing ANALYSIST client...")
        analysist = ANALYSIST()
    else:
        logger.debug("Using pre-initialized ANALYSIST client")
    ...
    if method == "tools":
        return _analyze_with_tools(
            image_path, image_bytes, filename, analysist, client, pipeline_start
        )
    else:
        return _analyze_manual(
            image_path, image_bytes, filename, analysist, client, pipeline_start
        )
...
def _analyze_with_tools(
    ...
    food_list: FoodList = analysist.analyze_food_with_tools(
    ...
    logger.info(
        "Pipeline tool-calling analysis complete: %d dish(es) in %.1fs",
        len(food_list.dishes),
...
def _analyze_manual(
    ...
    # Step 1: ANALYSIST image analysis
    logger.title("Step 1/2: ANALYSIST Image Analysis")
    step_start = time.time()
    food_list: FoodList = analysist.analyze_food(
        image_path=image_path, image_bytes=image_bytes, filename=filename
    )
    logger.info(
        "Step 1/2 complete: detected %d dish(es) in %.1fs",
        len(food_list.dishes),
        time.time() - step_start,
    )
    ...
    for food_idx, food in enumerate(food_list.dishes, 1):
        logger.info(
            "Processing dish %d/%d: %s", food_idx, len(food_list.dishes), food.name
        )
        ...
        for ing_idx, ing in enumerate(food.ingredients, 1):
            ...
            if usda_100g and any(
                usda_100g.get(k, 0) > 0 for k in ["calories", "protein", "carbs", "fat"]
            ):
                ...
                logger.debug(
                    "  USDA data unavailable/zeros. Keeping model's estimated_nutritions."
                )
        ...
        dish_total_adjusted = adjust_nutrition_for_cooking_method(
            dish_total_raw, food.cooking_method
        )

        food.nutritions = NutritionInfo(**dish_total_adjusted)

        logger.info(
            "Dish '%s' total: cal=%.1f pro=%.1f carb=%.1f fat=%.1f",
            food.name,
            dish_total_adjusted["calories"],
            dish_total_adjusted["protein"],
            dish_total_adjusted["carbs"],
            dish_total_adjusted["fat"],
        )
...
def print_report(results: dict):
    ...
    for food in dishes:
        ...
        if ingredients:
            ...
            print("-" * 95)
            print(
                f"📊 TỔNG CỘNG: Cân nặng={total_weight}g | Cal={t.get('calories', 0):.1f} | Pro={t.get('protein', 0):.1f}g | Carb={t.get('carbs', 0):.1f}g | Fat={t.get('fat', 0):.1f}g"
            )
        else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            "Usage: python -m app.scripts.food_analyzer <image_path> [--method tools|manual]"
        )
        sys.exit(1)
    ...
    results = analyze_food_nutrition(img, method=method)
